...
class Zidda {
  ...
  private async SingleAD() {
    for (var i = 0; i < this.Links.length; i++) {
      ...
      let Number_Of_Rooms = await this.page!.$eval(
      ...
      let meteer = await this.page!.$eval(
      ...
      let PhoneNumber = await this.page!.$eval(
        "#mat-dialog-1 > app-author-phone-dialog > div > mat-dialog-content > section.flex.flex-col.items-center.gap-3 > a",
        (el) => {
          return (el as HTMLElement).innerText;
        }
      );
